...
emoticonhandler::emoticonhandler(QWidget *parent) :
	QWidget(parent) {
	ui.setupUi(this);
        scheme=singleton<snpsettings>().map["smileyscheme"].toString();
	if(scheme.isEmpty()){
		scheme="default";
	}
...
void emoticonhandler::add() {
	if (ui.lineEdit->text().simplified() != "") {
		if (ui.listWidget->findItems(ui.lineEdit->text().simplified(),
				Qt::MatchCaseSensitive).size() == 0) {
			...
			filemap[scheme][ui.lineEdit->text().simplified()]
					= QFileInfo(s).fileName();
			QListWidgetItem *item = new QListWidgetItem(QIcon(s),
...
void emoticonhandler::indexofcbchanged(const QString&) {
	...
	ui.listWidget->clear();
	scheme = ui.cbscheme->currentText();
        singleton<snpsettings>().map["smileyscheme"]=scheme;
