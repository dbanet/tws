...
ircnet::ircnet(QString s, QObject *parent) :
	...
	QStringList sl;
	nick = s;
	sl = inihandler.stringlistfromini("[irc servernick]");
	if (!sl.isEmpty())
		servernick = sl.first();
	else
		servernick
				= inihandler.stringlistfromini("[http wormnet server]").first().remove(
						"http://");
	servermessageindicator = QString(":") + servernick + " ";
...
void ircnet::tcpread() {
	...
	foreach(QString s,sl) {
			if (s.startsWith(servermessageindicator))
				readservermassege(s.remove(servermessageindicator));
...
void ircnet::readusermessage(QString &s) {
	...
	switch (mapusercommand[command]) { //mapusercommand["QUIT"]=1;
	//mapusercommand["PRIVMSG"]=2;
	//mapusercommand["PART"]=3;
	//mapusercommand["JOIN"]=4;
	//mapusercommand["NOTICE"]=5;
...
void ircnet::readservermassege(QString s) {
	QStringList sl = s.split(" ");
	Q_ASSERT(sl.size()>=4);
	int command = sl.takeFirst().toInt();
	sl.takeFirst();
	QString channel;
	switch (command) {
